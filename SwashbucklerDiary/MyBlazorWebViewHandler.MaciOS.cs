...
#nullable disable
    public class MyBlazorWebViewHandler : BlazorWebViewHandler
    {
        ...
        private BlazorWebViewHandlerReflection Base
            => _base ??= new(this);
        ...
        protected override WKWebView CreatePlatformView()
        {
            ...
            //    webview.SetValueForKey(NSObject.FromObject(DeveloperToolsEnabled), new NSString("inspectable"));
            //}
        ...
        {
            private readonly MyBlazorWebViewHandler _webViewHandler;

            public SchemeHandler(MyBlazorWebViewHandler webViewHandler)
            {
    ...
    public class BlazorWebViewHandlerReflection
    {
        ...
        private string _blazorInitScript;

        private Uri _appOriginUri;
        ...
        public string BlazorInitScript => GetBlazorInitScript();

        public Uri AppOriginUri => GetAppOriginUri();
        ...
        private ILogger GetLogger()
        {
            if (_logger == null)
            {
                ...
                _logger = (ILogger)property?.GetValue(_blazorWebViewHandler);
            }

            return _logger;
        }

        private string GetBlazorInitScript()
        {
            if (_blazorInitScript == null)
            {
                var property = Type.GetField("BlazorInitScript", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
                _blazorInitScript = (string)property?.GetValue(_blazorWebViewHandler);
            }

            return _blazorInitScript;
        }

        private Uri GetAppOriginUri()
        {
            if (_appOriginUri == null)
            {
                var property = Type.GetField("AppOriginUri", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
                _appOriginUri = (Uri)property?.GetValue(_blazorWebViewHandler);
            }

            return _appOriginUri;
        }
