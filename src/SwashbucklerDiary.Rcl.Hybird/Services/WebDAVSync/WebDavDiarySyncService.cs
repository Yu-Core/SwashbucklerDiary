...
namespace SwashbucklerDiary.Rcl.Services
{
    public class WebDavDiarySyncService : IDiarySyncService
    {
        ...
        public async Task<bool> HasLocalChangesAsync()
        {
            ...
            diaries = await ExcludeAppliedRemoteDiariesAsync(diaries).ConfigureAwait(false);
            if (diaries.Count != 0)
            {
                return true;
            }
            ...
            foreach (var change in batch.Diaries)
            {
                if (change.Operation == OperationUpsert && change.Diary is not null)
                {
                    ...
                    if (local is null)
                    {
                        ...
                        await AddRemoteAppliedAsync(batch, path, change).ConfigureAwait(false);
                        result.Pulled++;
                    }
        ...
        private async Task<List<DiaryModel>> ExcludeAppliedRemoteDiariesAsync(List<DiaryModel> diaries)
        {
            if (diaries.Count == 0)
            {
                return diaries;
            ...
            var ids = diaries.Select(it => it.Id.ToString()).ToList();
            var appliedRemoteUpdates = await Db.Queryable<SyncRemoteAppliedModel>()
                .Where(it => it.EntityName == EntityDiary && ids.Contains(it.EntityId))
                .ToListAsync()
                .ConfigureAwait(false);
            if (appliedRemoteUpdates.Count == 0)
            {
                return diaries;
            }

            var appliedRemoteUpdateTimes = appliedRemoteUpdates
                .GroupBy(it => it.EntityId)
                .ToDictionary(it => it.Key, it => it.Max(a => a.RemoteUpdateTime));

            return diaries
                .Where(diary => !appliedRemoteUpdateTimes.TryGetValue(diary.Id.ToString(), out var remoteUpdateTime)
                    || diary.UpdateTime > remoteUpdateTime)
                .ToList();
        }

        private Task AddRemoteAppliedAsync(SyncBatch batch, string path, SyncDiaryChange change)
        {
            DateTime now = DateTime.Now;
            return Db.Storageable(new SyncRemoteAppliedModel()
            {
                Id = Guid.NewGuid(),
                EntityName = EntityDiary,
                EntityId = change.Id.ToString(),
            ...
            .WhereColumns(it => new { it.EntityName, it.EntityId, it.RemoteUpdateTime })
            .ExecuteCommandAsync();
        }
        ...
        {
            Db.CodeFirst.InitTables<SyncStateModel, SyncTombstoneModel, SyncConflictModel, SyncRemoteAppliedModel>();
        }
